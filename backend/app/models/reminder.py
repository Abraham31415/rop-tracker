...
class Reminder(Base):
    ...
    reminder_type = Column(Enum(ReminderType, values_callable=lambda x: [e.name for e in x]), nullable=False)
    trigger = Column(Enum(ReminderTrigger, values_callable=lambda x: [e.name for e in x]), nullable=False)
    language = Column(String, nullable=False, default="english")

    recipient_phone = Column(String, nullable=True)
    message_body = Column(Text, nullable=True)

    status = Column(Enum(ReminderStatus, values_callable=lambda x: [e.name for e in x]), default=ReminderStatus.PENDING)
