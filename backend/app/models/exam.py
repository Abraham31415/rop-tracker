...
class Exam(Base):
    ...
    # Right eye
    right_zone = Column(Enum(Zone, values_callable=lambda x: [e.name for e in x]), nullable=True)
    right_stage = Column(Enum(Stage, values_callable=lambda x: [e.name for e in x]), nullable=True)
    right_plus = Column(Enum(PlusDisease, values_callable=lambda x: [e.name for e in x]), default=PlusDisease.NONE)

    # Left eye
    left_zone = Column(Enum(Zone, values_callable=lambda x: [e.name for e in x]), nullable=True)
    left_stage = Column(Enum(Stage, values_callable=lambda x: [e.name for e in x]), nullable=True)
    left_plus = Column(Enum(PlusDisease, values_callable=lambda x: [e.name for e in x]), default=PlusDisease.NONE)

    # Worst finding (used for scheduling — system auto-derives this)
    worst_zone = Column(Enum(Zone, values_callable=lambda x: [e.name for e in x]), nullable=True)
    worst_stage = Column(Enum(Stage, values_callable=lambda x: [e.name for e in x]), nullable=True)
    ...
    vf_right_csm = Column(Enum(VFCSM, name='vfcsm', values_callable=lambda x: [e.name for e in x]), nullable=True)
    vf_right_teller_acuity = Column(Float, nullable=True)
    vf_right_vep = Column(Float, nullable=True)
    vf_left_fixation = Column(Enum(VFFixation, name='vffixation', values_callable=lambda x: [e.name for e in x]), nullable=True)
    vf_left_following = Column(Enum(VFFollowing, name='vffollowing', values_callable=lambda x: [e.name for e in x]), nullable=True)
    vf_left_csm = Column(Enum(VFCSM, name='vfcsm', values_callable=lambda x: [e.name for e in x]), nullable=True)
    vf_left_teller_acuity = Column(Float, nullable=True)
    vf_left_vep = Column(Float, nullable=True)
    vf_nystagmus = Column(Enum(Nystagmus, name='nystagmus', values_callable=lambda x: [e.name for e in x]), nullable=True)
