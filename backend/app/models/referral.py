...
class Referral(Base):
    ...
    reason = Column(Enum(ReferralReason, values_callable=lambda x: [e.name for e in x]), nullable=False)
    referral_date = Column(Date, nullable=False)
    status = Column(Enum(ReferralStatus, values_callable=lambda x: [e.name for e in x]), default=ReferralStatus.PENDING)
